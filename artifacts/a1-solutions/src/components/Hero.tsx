import { motion } from 'framer-motion';

const Hero = () => {
  const scrollToSection = (id: string) => {
    document.getElementById(id)?.scrollIntoView({ behavior: 'smooth' });
  };

  return (
    <section
      className="relative flex items-center justify-center overflow-hidden bg-gradient-to-br from-[#0D0D0D] via-[#1A1A1A] to-[#2D2D2D] border-b-4 border-[#FF8C00]"
      style={{ paddingTop: 'clamp(3rem, 8vw, 6rem)', paddingBottom: 'clamp(3rem, 8vw, 6rem)' }}
    >
      {/* Background gears — clipped inside section, hidden on print */}
      <div
        className="absolute inset-0 pointer-events-none opacity-[0.07] gear-bg select-none"
        aria-hidden="true"
        data-gear="true"
        style={{ overflow: 'hidden' }}
      >
        <span
          style={{
            position: 'absolute',
            fontSize: 'clamp(8rem, 20vw, 22rem)',
            top: '-6rem',
            left: '-5rem',
            display: 'inline-block',
            animation: 'spin 35s linear infinite',
            willChange: 'transform',
            backfaceVisibility: 'hidden',
            transformOrigin: 'center center',
          }}
        >
          ⚙️
        </span>
        <span
          style={{
            position: 'absolute',
            fontSize: 'clamp(6rem, 15vw, 18rem)',
            bottom: '-5rem',
            right: '-4rem',
            display: 'inline-block',
            animation: 'spin 45s linear infinite reverse',
            willChange: 'transform',
            backfaceVisibility: 'hidden',
            transformOrigin: 'center center',
          }}
        >
          ⚙️
        </span>
      </div>

      <div className="relative z-10 w-full max-w-3xl mx-auto text-center" style={{ padding: '0 clamp(1rem, 5vw, 2.5rem)' }}>

        {/* Eyebrow */}
        <motion.p
          className="font-['Barlow_Condensed'] text-[#B8B8B8] font-medium uppercase tracking-widest"
          style={{ fontSize: 'clamp(0.8rem, 2vw, 1.1rem)', marginBottom: 'clamp(0.5rem, 2vw, 1rem)' }}
          initial={{ opacity: 0, y: 16 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5, delay: 0.1 }}
        >
          You Have a Problem?
        </motion.p>

        {/* Main headline */}
        <motion.h1
          className="font-['Barlow_Condensed'] font-bold uppercase text-white"
          style={{
            fontSize: 'clamp(1.75rem, 6vw + 0.5rem, 4.5rem)',
            lineHeight: 1.05,
            letterSpacing: 'clamp(1px, 0.3vw, 4px)',
            marginBottom: 'clamp(0.25rem, 1vw, 0.5rem)',
          }}
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, delay: 0.2 }}
        >
          We Are The
        </motion.h1>

        <motion.h1
          className="font-['Barlow_Condensed'] font-bold uppercase text-[#FF8C00]"
          style={{
            fontSize: 'clamp(3rem, 12vw + 0.5rem, 8rem)',
            lineHeight: 0.95,
            letterSpacing: 'clamp(2px, 0.5vw, 8px)',
            textShadow: '0 0 40px rgba(255,140,0,0.35)',
            marginBottom: 'clamp(1rem, 3vw, 2rem)',
          }}
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, delay: 0.3 }}
        >
          Solution
        </motion.h1>

        {/* Sub-tagline */}
        <motion.p
          className="text-[#B8B8B8] leading-relaxed mx-auto"
          style={{
            fontSize: 'clamp(0.875rem, 1.5vw + 0.2rem, 1.125rem)',
            maxWidth: '36rem',
            marginBottom: 'clamp(1.5rem, 4vw, 2.5rem)',
          }}
          initial={{ opacity: 0, y: 16 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5, delay: 0.4 }}
        >
          24/7 Mobile Mechanic — Emergency Roadside Repair &amp; Automotive Solutions in Texas &amp; Louisiana
        </motion.p>

        {/* CTA buttons — stacked on mobile, side-by-side on sm+ */}
        <motion.div
          className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-stretch"
          style={{ marginBottom: 'clamp(1.5rem, 4vw, 2.5rem)' }}
          initial={{ opacity: 0, y: 16 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5, delay: 0.5 }}
        >
          {/* Primary CTA — tap-to-call */}
          <motion.a
            href="tel:14097797780"
            className="flex items-center justify-center gap-3 bg-[#FF8C00] text-[#0D0D0D] font-bold border-[3px] border-[#FFA500] shadow-[0_4px_20px_rgba(255,140,0,0.4)]"
            style={{
              minHeight: '60px',
              padding: 'clamp(0.75rem, 2vw, 1.1rem) clamp(1.25rem, 4vw, 2.5rem)',
              borderRadius: '2px',
            }}
            whileHover={{ scale: 1.04, boxShadow: '0 6px 28px rgba(255,140,0,0.55)' }}
            whileTap={{ scale: 0.97 }}
            aria-label="Call Bill at 1-409-779-7780"
          >
            <span className="text-2xl flex-shrink-0" aria-hidden="true">📞</span>
            <div className="flex flex-col items-start leading-tight">
              <span className="font-['Barlow_Condensed'] uppercase tracking-[2px]" style={{ fontSize: 'clamp(0.65rem, 1.2vw, 0.75rem)' }}>
                Call Bill Now — 24/7
              </span>
              <span className="font-['Barlow_Condensed'] font-bold tracking-wide" style={{ fontSize: 'clamp(1.1rem, 3vw, 1.5rem)' }}>
                1-409-779-7780
              </span>
            </div>
          </motion.a>

          {/* Secondary CTA */}
          <motion.button
            className="flex items-center justify-center font-['Barlow_Condensed'] font-bold uppercase tracking-widest text-[#FF8C00] border-[3px] border-[#FF8C00] bg-transparent hover:bg-[#FF8C00] hover:text-[#0D0D0D] transition-colors duration-200"
            style={{
              minHeight: '60px',
              padding: 'clamp(0.75rem, 2vw, 1.1rem) clamp(1.25rem, 4vw, 2rem)',
              fontSize: 'clamp(0.875rem, 1.5vw, 1rem)',
              borderRadius: '2px',
            }}
            whileHover={{ scale: 1.04 }}
            whileTap={{ scale: 0.97 }}
            onClick={() => scrollToSection('contact')}
          >
            Request Service
          </motion.button>
        </motion.div>

        {/* Trust badges — always visible, wraps gracefully */}
        <motion.div
          className="flex flex-wrap gap-x-6 gap-y-3 justify-center"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.4, delay: 0.65 }}
        >
          {[
            { icon: '⚡', text: '24/7 Available' },
            { icon: '🚗', text: 'Mobile Service' },
            { icon: '🔧', text: 'Expert Repairs' },
          ].map((badge) => (
            <div
              key={badge.text}
              className="flex items-center gap-2 text-[#B8B8B8] font-['Barlow_Condensed'] font-semibold uppercase tracking-wide"
              style={{ fontSize: 'clamp(0.75rem, 1.5vw, 0.9rem)' }}
            >
              <span aria-hidden="true" style={{ fontSize: 'clamp(1rem, 2.5vw, 1.25rem)' }}>{badge.icon}</span>
              <span>{badge.text}</span>
            </div>
          ))}
        </motion.div>
      </div>
    </section>
  );
};

export default Hero;
